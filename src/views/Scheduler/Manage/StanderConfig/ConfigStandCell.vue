<template>
  ...
  <stand-convergence :dialog-table-visible="state.tableVisible" @close="closeModal" @give-code="getCode" :table-name="taskDefinition.taskWork"/>
</template>
...
<script>
  ...
  import StandConvergence from "./StandConvergence.vue";

  export default defineComponent({
  components: { StandConvergence },
    name: "cellFrom",
    props: {
      visible: {
        type: Boolean,
    ...
    setup(props, {emit}) {
      ...
      watch([visible, code, projectCode, workName, taskCode, definition, id],(newval,oldval) => {
        standVisible.value = newval[0]
        state.code = newval[1]
        state.projectCode = newval[2]
        state.name = newval[3]
        // let arr = {
        //   code : newval[4],
        //   projectCode : newval[2],
        //   nodeId :newval[4],
        // }
        // console.log(arr);
        // Object.assign(taskDefinition,arr)
