<template>
  ...
  >
    <div class="demo-drawer__content">
      <el-form ref="Form" :model="taskDefinition" label-width="100px" :rules="state.rules">
        <el-form-item label="节点名称" prop="name">
          <el-input v-model="taskDefinition.name" autocomplete="off" />
        </el-form-item>
        <el-form-item label="描述" prop="description">
          <el-input v-model="taskDefinition.description" type="textarea" />
        </el-form-item>
        <el-form-item label="超时失败" prop="timeoutFlag">
          <el-switch
            v-model="taskDefinition.timeoutFlag"
            active-text="启用"
            inactive-text="禁用"
            active-value="OPEN"
        ...
        <el-form-item label="作业名" prop="taskWork">
          <el-input v-model="taskDefinition.taskWork" class="flex-1" disabled />
          <el-button type="primary" class="flex ml-20" @click="chooseWork">
            选择
          </el-button>
        </el-form-item>
      </el-form>
  ...
  <monitor-convergence :dialog-table-visible="state.tableVisible" :table-name="taskDefinition.taskWork" @close="closeModal" @give-code="getCode" />
</template>
...
<script>
  ...
  import MonitorConvergence from "./MonitorConvergence.vue";

  export default defineComponent({
    name: "CellFrom",
  components: { MonitorConvergence },
    props: {
        visible: {
        type: Boolean,
        default: false
    ...
    emits:['close', 'getMonitor'],
    setup(props, { emit }) {
      const { proxy } = getCurrentInstance();
      const { visible, code, projectCode, workName, taskCode, definition, id, currentId } = toRefs(props)
      ...
      watch([visible, code, projectCode, workName, taskCode, definition, id, currentId],(newval,oldval) => {
        monitorVisible.value = newval[0]
        state.code = newval[1]
        ...
        taskDefinition.currentId = newval[7]
        let ong = {
          name: '',
          description: '',
          timeoutFlag: "CLOSE",
      ...
      const onCommit = () => {
        proxy.$refs.Form.validate(valid => {
          if(valid) {
            emit("getMonitor", taskDefinition);
