<template>
  ...
  <div class="flex justify-content-between align-items-center mb-5">
    <div>
      <el-button v-if="showAddButton" class="el-icon-plus c-add-button" @click="onAdd">
        新增
      </el-button>
      <el-button v-if="showPatchDeleteButton" class="el-icon-delete c-delete-button" :disabled="state.tableSelection.length === 0" @click="onDelete()">
        删除
      </el-button>
    </div>
    <div class="flex">
      ...
      <el-tooltip v-if="showColumnConfig" effect="dark" content="展示列配置" placement="top">
        <div>
          <el-popover placement="bottom" trigger="click">
            <template #reference>
              <i class="el-icon-s-tools green-color cursor-pointer" />
            </template>
